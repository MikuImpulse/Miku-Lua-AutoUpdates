...
if getName ~= getWorkingDirectory()..'/MikuProjectReborn_beta.lua' then
    os.rename(getName, getWorkingDirectory()..'/MikuProjectReborn_beta.lua')
    thisScript():reload()
end
--------Библиотеки--------
...
-- auto updates
local lmPath = "MikuProjectReborn_beta.lua"
local lmUrl = "https://raw.githubusercontent.com/MikuImpulse/Miku-Lua-AutoUpdates/main/MikuProjectReborn_beta.lua"
local updfont = {}
-- togglebutton
local AI_TOGGLE = {}
        ...
        if result and isCharOnScreen(char) and char ~= PLAYER_PED then
            ...
            if settings.ESP.enabled_nicks[0] then
                ...
                if hr then
                    ...
                    local nametag = nickname .. ' [' .. tostring(id) .. '] - ' .. string.format("%.0f", sampGetPlayerHealth(id)) .. 'hp ' .. string.format("%.0f", sampGetPlayerArmor(id)) .. 'ap'
